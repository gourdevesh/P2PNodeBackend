...
export const getCryptoAd = async (req, res) => {
    try {
        ...
        const { per_page = 10, user_id, txn_type, cryptocurrency, paymentMethod, offerLocation, traderLocation, activeTrader, activeCryptoOffer, acceptedOffer, search } = req.query;
        ...
        if (offerLocation)
            where.country = offerLocation.toLowerCase();
        if (activeCryptoOffer)
            where.is_active = activeCryptoOffer === "true";
        if (acceptedOffer)
            where.is_accepted = acceptedOffer === "true";
        ...
        // ✅ Analytics counts
        const [totalCryptoAds, totalBuyCryptoAds, totalSellCryptoAds, totalActiveAds, totalAcceptedAds,] = await Promise.all([
        ...
        const pagination = {
            ...
            next_page_url: page < Math.ceil(totalFilteredDataCount / perPage)
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (cryptoAdDetails.is_active === is_active) {
                throw new Error(`Crypto Advertisement is already ${is_active ? "active" : "inactive"}.`);
            }
            if (cryptoAdDetails.is_accepted) {
                throw new Error("The selected ad is currently involved in an active trade. Please try again later.");
            }
...
export const completeRequestedPendingTrade = async (req, res) => {
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const sellerRemainingAmount = sellerAvailableAmount.minus(new Prisma.Decimal(tradeDetails.hold_asset));
            const sellerTxnData = {
                user_id: tradeDetails.seller_id,
                txn_type: "internal",
            ...
            await tx.web3_wallets.update({
                ...
                data: {
                    withdrawal_amount: new Prisma.Decimal(sellerWallet.withdrawal_amount).plus(new Prisma.Decimal(tradeDetails.hold_asset)),
                    remaining_amount: sellerRemainingAmount,
                    hold_asset: new Prisma.Decimal(sellerWallet.hold_asset).minus(new Prisma.Decimal(tradeDetails.hold_asset)),
                    created_at: new Date(),
                    updated_at: new Date()
                },
            });
            ...
            await tx.web3_wallets.update({
                ...
                data: {
                    deposit_amount: new Prisma.Decimal(buyerWallet.deposit_amount).plus(paidAmount),
                    remaining_amount: buyerRemainingAmount,
                    internal_deposit: new Prisma.Decimal(buyerWallet.internal_deposit).plus(paidAmount),
                    created_at: new Date(),
                    updated_at: new Date()
                },
            });
